...
const config = require(options.configuration || `${homedir}/pm-config.js`)

const getProjects = {
  type: 'multiselect',
  name: 'projects',
  message: 'Which projects would you like to update?',
  choices: ['All', ...config.projects.map(p => p.name)],
  initial: 'All'
}
...
const getProjects = async () => {
  let { projects: selectedProjects, baseBranch, newBranch } = await prompt([
    getProjects,
    getBaseBranch,
    getNewBranchName
  ])

  return selectedProjects.includes('All') ? config.projects : config.projects.filter(p => selectedProjects.includes(p.name))
}
